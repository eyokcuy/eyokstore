...
$db = getDBConnection();
$pageTitle = 'Edit Game Item';

$id = intval($_GET['id'] ?? 0);
$stmt = $db->prepare("SELECT * FROM game_items WHERE id = ?");
$stmt->execute([$id]);
$item = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $db->prepare("UPDATE game_items SET game_id = ?, name = ?, price = ?, status = ? WHERE id = ?");
        $stmt->execute([$game_id, $name, $price, $status, $id]);
        header('Location: index.php');
        exit;
    }
}
...
<main class="lg:pl-64 pt-16">
    <div class="p-4 lg:p-8 max-w-2xl">
        <div class="mb-8">
            ...
            <h1 class="text-2xl font-bold text-gray-900">Edit Game Item</h1>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <form action="" method="POST" class="space-y-6">
                <div>
                    ...
                    <select name="game_id" required class="w-full bg-gray-50 border <?php echo isset($errors['game_id']) ? 'border-red-500 ring-1 ring-red-500' : 'border-gray-200'; ?> rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                        <?php foreach ($games as $game): ?>
                            <option value="<?php echo $game['id']; ?>" <?php echo $game['id'] == $game_id ? 'selected' : ''; ?>>
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Item Amount/Name</label>
                    <input type="text" name="name" value="<?php echo htmlspecialchars($name); ?>" required class="w-full bg-gray-50 border <?php echo isset($errors['name']) ? 'border-red-500 ring-1 ring-red-500' : 'border-gray-200'; ?> rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="price" value="<?php echo $price; ?>" required step="0.01" class="w-full bg-gray-50 border <?php echo isset($errors['price']) ? 'border-red-500 ring-1 ring-red-500' : 'border-gray-200'; ?> rounded-lg pl-10 pr-4 py-2.5 focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                    </div>
                ...
                <div class="pt-4 flex gap-3">
                    <button type="submit" class="flex-1 bg-indigo-600 text-white font-bold py-3 rounded-lg hover:bg-indigo-700 transition-colors shadow-sm">Update Item</button>
