<?php
/**
 * GameTopUp Pro - Edit Transaction
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $updateStmt = $db->prepare("UPDATE transactions SET 
                                    game_id = :game_id, 
        ...
        $_SESSION['flash'] = ['type' => 'success', 'message' => 'Transaction updated successfully.'];
        header('Location: index.php');
        exit;
    }
}
...
<main class="lg:ml-64 pt-16 min-h-screen">
    <div class="p-4 lg:p-8 max-w-2xl" x-data="{ quantity: <?php echo intval($_POST['quantity'] ?? $transaction['quantity']); ?>, price: <?php echo floatval($_POST['price'] ?? $transaction['price']); ?> }">
        <!-- Page Header -->
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Edit Transaction</h1>
                <p class="text-gray-500 mt-1">Invoice: <?php echo htmlspecialchars($transaction['invoice_code']); ?></p>
            </div>
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <select id="game_id" name="game_id" class="w-full px-4 py-2 border <?php echo isset($errors['game_id']) ? 'border-red-500' : 'border-gray-300'; ?> rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">
                            ...
                            <?php 
                            $selectedGameId = $_POST['game_id'] ?? $transaction['game_id'];
                            foreach ($games as $g): 
                            ?>
                                <option value="<?php echo $g['id']; ?>" <?php echo ($selectedGameId == $g['id']) ? 'selected' : ''; ?>><?php echo htmlspecialchars($g['name']); ?></option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" id="customer_name" name="customer_name" value="<?php echo htmlspecialchars($_POST['customer_name'] ?? $transaction['customer_name']); ?>" maxlength="100" class="w-full px-4 py-2 border <?php echo isset($errors['customer_name']) ? 'border-red-500' : 'border-gray-300'; ?> rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">
                        ...
                        <div>
                            ...
                            <input type="text" id="contact" name="contact" value="<?php echo htmlspecialchars($_POST['contact'] ?? $transaction['contact']); ?>" maxlength="50" class="w-full px-4 py-2 border <?php echo isset($errors['contact']) ? 'border-red-500' : 'border-gray-300'; ?> rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">
                    ...
                    <div>
                        ...
                        <input type="text" id="game_uid" name="game_uid" value="<?php echo htmlspecialchars($_POST['game_uid'] ?? $transaction['game_uid']); ?>" maxlength="50" class="w-full px-4 py-2 border <?php echo isset($errors['game_uid']) ? 'border-red-500' : 'border-gray-300'; ?> rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">
                    ...
                    <div>
                        ...
                        <input type="text" id="item_name" name="item_name" value="<?php echo htmlspecialchars($_POST['item_name'] ?? $transaction['item_name']); ?>" maxlength="100" class="w-full px-4 py-2 border <?php echo isset($errors['item_name']) ? 'border-red-500' : 'border-gray-300'; ?> rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">
                    ...
                    <div>
                        ...
                        <select id="status" name="status" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none">
                            <?php $selectedStatus = $_POST['status'] ?? $transaction['status']; ?>
                            <option value="pending" <?php echo ($selectedStatus === 'pending') ? 'selected' : ''; ?>>Pending</option>
                            <option value="success" <?php echo ($selectedStatus === 'success') ? 'selected' : ''; ?>>Success</option>
                    ...
                    <div>
                        ...
                        <textarea id="notes" name="notes" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none"><?php echo htmlspecialchars($_POST['notes'] ?? $transaction['notes']); ?></textarea>
                    </div>
